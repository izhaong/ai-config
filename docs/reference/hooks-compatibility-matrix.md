...
本文定义 `ai-config` 在 Cursor / Claude / Codex 三端的 Hook 下发兼容策略，目标是让同一份 `.ai-config/hooks.json` 在不同平台可预测执行。

## 统一原则

- 源资产唯一入口：`.ai-config/hooks.json` + `.ai-config/hooks/`
- 平台差异收敛在 `ai-config-core` 的 `hook_adapter`，不在平台配置手工分叉维护
...
- 文件编辑事件：`Edit|Write`
- MCP 事件：`mcp__.*`
- Cursor 的 shell 默认 matcher 维持 `ai-config\\b`，兼容历史行为
...
2. 重新从 `.ai-config/hooks.json` 生成旧策略
