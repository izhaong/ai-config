# agents-manager 桌面端 — 项目排期

> 状态：**v0.2** · 2026-06-12（M1 Approved；Q-1..Q-8 拍板同步；crate 命名/前端框架与 plan 对齐）
> 范围：本项目交付的时间 / 任务 / 依赖 / 资源
> 上游：`PRD.md` v0.3 Approved（§12.4 决策日志）·`.claude/plans/agents-manager/20260612_agents-manager_rust-desktop.plan.md`（§8.1 已决 / §8.2 待决）
> 下游：`ARCHITECTURE.md`（组件 / 模块边界）·`DESIGN.md`（GUI 信息架构）
> 修订：每周末更新（一次大迭代后大改一次）
> **CLI 二进制名**：`agents-manager`（2026-06-12 统一；旧 `agents-managerd` 措辞已废止，参见 PRD §6.1 退码表）
...
| | **小计** | **2.5** | |
| **1 — CLI** | `agents-manager-core` 模块（fs / 模板 / 链接） | 4.0 | junction 跨平台 |
| | `agents-manager-cli` 子命令树（11 个顶层） | 3.0 | clap 派生 + 退出码 |
| | `mcp` 逐项子命令组 | 2.0 | 旧整文件 → 逐项迁移器 |
| | `secrets` 子命令组 | 1.5 | 0600 写入 + 引导 |
| | 行为对齐测试（旧 install.sh vs 新 CLI） | 2.0 | 跨平台 fixture |
| | **crates.io 发布准备**（Q-1 第二通道）：`cargo login`、`cargo publish --dry-run`、发布 `agents-manager = "0.1"`、验证 tarball（Q-1 决策：Phase 1–2 之间上线） | 0.5 | crates.io 账号 + API token 需用户提供 |
| | **小计** | **13.0** | |
...
| | **brew tap 发布**（Q-1 第四通道）：建 `homebrew-zh-cloud` 仓，写 `agents-manager.rb` Formula，README 加 `brew install zh-cloud/agents-manager/agents-manager` 横幅 | 0.5 | 需先发 GH Release |
| | **自启动集成子命令**（Q-4 决策：M6 之后在 `install` 子命令里集成 launchd / systemd --user / Task Scheduler）：`agents-manager install --autostart` 子模式 | 1.0 | launchd plist / systemd user unit 模板 |
| | **小计** | **4.5** | |
...
- [ ] 创 `crates/` workspace（**7 个 crate，对齐 plan §2.2.1**）：
  - `crates/agents-manager-core`、`crates/agents-manager-cli`、`crates/agents-manager-daemon`
  - `crates/agents-manager-bus`、`crates/agents-manager-watcher`、`crates/agents-manager-store`
  - `apps/agents-manager-gui`（Tauri 壳，Phase 3 才起骨架；本阶段只建空目录）
...
- [ ] `.gitignore` 加 `target/` `.agents-manager/daemon.sock`
...
- [ ] `agents-manager-core` 模块：`fs`（链接 + junction 抽象）
- [ ] `agents-manager-core` 模块：`template`（minijinja + 原子 rename）
- [ ] `agents-manager-core` 模块：`config`（TOML / serde）
- [ ] `agents-manager-core` 模块：`error`（thiserror + anyhow）
- [ ] 单元测试：链接幂等 / 模板变量替换 / 0600 写入

### W4 (2026-07-07 ~ 07-13) — Phase 1.b CLI

- [ ] `agents-manager-cli`：clap 派生 11 个顶层子命令
- [ ] `install` / `uninstall` / `sync` / `status` / `list` / `show` / `doctor`
- [ ] 全局 `--json` / `--quiet` 实现
- [ ] 退出码 0/2/3/4/5
- [ ] 集成测试：干净 `$HOME` 下，`./install.sh --all` 与 `agents-manager install --all` 行为对齐
...
**Phase 1 exit criteria**：4 项 A-1 / A-3 / A-7 / A-8（PRD §7）验收过；旧 `install.sh` 退化为一行 wrapper（`exec cargo run -p agents-manager-cli --quiet -- install "$@"`），`scripts/*.mjs` 删除。

### W6 (2026-07-21 ~ 07-27) — Phase 2.a 守护进程核心

- [ ] `agents-manager-daemon` crate
...
- [ ] Unix socket：`~/.config/agents-manager/daemon.sock`
- [ ] JSON 帧协议（4 字节大端长度前缀）
- [ ] `agents-manager events --follow` 订阅 CLI
- [ ] `agents-manager status --watch` 也走 socket
...
- [ ] `apps/agents-manager-gui` crate + Tauri 2 + **React 18**（Q-2 拍板；覆盖原 "Solid" 推荐）
...
- [ ] secrets 列表（**不**显示明文值）
- [ ] `webui/data/agents-manager.db` → 新 SQLite 一次性迁移器
...
**Phase 4 exit criteria**：`git grep -E '\.sh$|\.mjs$|install\.sh' -- ':!target' ':!node_modules'` 在 `agents-manager/` 下 0 命中；A-2 / A-12 验收过。
...
- **2026-06-12 v0.2** — 同步 8 项已决决策（PRD §12.4）；Solid→React；crate 命名 5→7 与 plan §2.2.1 对齐；补 crates.io / 预编译包 CI / brew tap / i18n 资源骨架 / 自启动集成子任务；CLI 二进制 `agents-managerd` 统一为 `agents-manager`；总计 41.5→46.0 人天
