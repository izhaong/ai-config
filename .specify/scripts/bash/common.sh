...
check_feature_branch() {
    ...
    # agents-manager Git Flow: feat/*、fix/* 等须配合 SPECIFY_FEATURE 指向 specs/ 子目录
    ...
    if [[ ! "$branch" =~ ^[0-9]{3}- ]] && [[ ! "$branch" =~ ^[0-9]{8}-[0-9]{6}- ]]; then
        ...
        echo "Use: 001-feature-name | feat/issue-name (with SPECIFY_FEATURE) | create-new-feature.sh" >&2
        return 1
    fi
