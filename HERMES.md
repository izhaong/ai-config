...
| 资产       | agents-manager 下发目标                                        | Hermes 官方消费                                                     | 作用域                       |
| ---------- | --------------------------------------------------------- | ------------------------------------------------------------------- | ---------------------------- |
...
自定义 `HERMES_SKILLS_DIR` 时，agents-manager 会在 deploy 后把该路径写入 `skills.external_dirs`，否则 Hermes 不会扫描。
...
| 操作         | agents-manager 命令                        | 目标                                                             |
| ------------ | ------------------------------------- | ---------------------------------------------------------------- |
| 单条 deploy  | `agents-manager mcp deploy <name> hermes`  | merge `mcp_servers.<name>`                                       |
| 单条 retract | `agents-manager mcp retract <name> hermes` | 移除 `mcp_servers.<name>`                                        |
| 遗留迁移     | `agents-manager mcp migrate-hermes`        | 将旧 `~/.hermes/mcp.json` 合并进 config.yaml 后 rename 为 `.bak` |

写入前会备份为 `config.yaml.agents-manager.bak.<unix_ts>`。YAML 全文件重序列化**可能丢失注释** — 请依赖备份回滚。
