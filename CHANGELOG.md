...
- **四层验证闭环**：`agents-manager-verify` 增加源码测试、临时 HOME 同步生命周期、已安装版本/只读状态门禁，以及 GUI/IDE/daemon 人工验收边界。
...
- **GUI 写入边界**：移除 Tauri 的 deploy/retract、平台间复制与冲突覆盖 command，避免 GUI 绕过 source-first ownership、digest 和 rollback 合同。GUI 保留源资产编辑与显式导入；平台投影请使用 `agents-manager sync --apply` 或 `agents-manager uninstall --apply`。
...
- **`install` / `sync --workspace`**（zh-cloud #42）：解析 `.gitmodules`，对父仓与已 checkout 子模块逐个下发；子仓无本地 `.agents-manager` 时继承父仓资产源。
...
- **资产下发不再生成 `.agents-manager-deploy.json`**：同步状态改由内容比对判定；仍兼容读取历史 marker 与 symlink。
...
- **GUI 本地打包**：`pnpm tauri:build` 自动加载 `~/.tauri/agents-manager.key` 签名 updater 产物。
...
- **Agent Skill**：`.cursor/skills/agents-manager-agent/SKILL.md` — 教 Agent 何时用 CLI `--json` 管理资产。
- **GUI 五平台对等**：agents-manager 与 4 IDE 并列；平台 icon deploy / retract / import / 跨平台硬拷贝（`deploy_from_platform`）。
- **链接态 `synced`**：外部安装（如 `npx skills`）内容一致但无 marker；点击可覆盖为 `linked`。
- **列表行「更新」**：从 agents-manager 平台副本重新 deploy 到各已激活平台。
- **GUI Skill 添加**：列表顶栏「+」；`npx skills add` 拉取到 agents-manager 源，IDE 目标则再 `deploy` 下发。
...
- **多平台 Skill 安装**：Marketplace / 批量添加时 IDE 平台改为「先写 agents-manager 源 → deploy」。
- **平台视图删除**：外部 / `synced` 的 Hermes（及其它 IDE）skill 可从平台目录删除，无需 agents-manager 源。
...
- 若曾依赖 agents-manager 写入的 `~/.hermes/mcp.json`，请运行 `agents-manager mcp migrate-hermes` 或手动合并到 `config.yaml`。
...
[0.4.1]: https://github.com/izhaong/agents-manager/releases/tag/v0.4.1
[0.3.3]: https://github.com/izhaong/agents-manager/releases/tag/v0.3.3
[0.4.0]: https://github.com/izhaong/agents-manager/releases/tag/v0.4.0
[0.3.2]: https://github.com/izhaong/agents-manager/releases/tag/v0.3.2
[0.3.1]: https://github.com/izhaong/agents-manager/releases/tag/v0.3.1
[0.3.0]: https://github.com/izhaong/agents-manager/releases/tag/v0.3.0
[0.2.0]: https://github.com/izhaong/agents-manager/releases/tag/v0.2.0
...
[0.1.0]: https://github.com/izhaong/agents-manager/releases/tag/v0.1.0
