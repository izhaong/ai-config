...
/// 扫描单个根目录下的 4 类资产。
///
/// `root` 通常是 `<project>/.ai-config/` 或 `~/.ai-config/`(全局资产根)。
...
/// 合并扫描:项目 `.ai-config/` 覆盖 ai-config 默认仓(同名覆盖,异名附加,PRD §3.2)。
///
/// - `project_root`:项目下 `.ai-config/` 目录路径
/// - `default_root`:全局资产根(`~/.ai-config/` 或测试用临时目录)
