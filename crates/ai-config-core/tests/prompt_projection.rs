...
fn project_roots(root: &Utf8Path) -> OverlayRoots {
    OverlayRoots {
        global: root.join("global/.ai-config"),
        workspace: Some(root.join("workspace/.ai-config")),
        project: root.join("project/.ai-config"),
    }
}
...
fn project_template_seeds_the_canonical_agents_prompt() {
    let prompt = project_template_root().join(".ai-config/prompts/AGENTS.md");

    assert!(
        prompt.is_file(),
        "the project template must seed the complete prompt at .ai-config/prompts/AGENTS.md"
    );
    assert!(
...
fn project_template_root_agents_is_an_exact_canonical_prompt_link() {
    ...
    assert_eq!(
        ...
        std::path::PathBuf::from(".ai-config/prompts/AGENTS.md"),
...
fn prompt_plans_one_project_entry_for_all_platforms() {
    ...
    write(
        &root.join("project/.ai-config/prompts/AGENTS.md"),
...
fn prompt_overlay_then_reapply_is_an_unchanged_noop() {
    ...
    let root = Utf8Path::from_path(temp.path()).unwrap();
    write(
        &root.join("global/.ai-config/prompts/AGENTS.md"),
        "global entry\n",
    );
    write(
        &root.join("project/.ai-config/prompts/AGENTS.md"),
