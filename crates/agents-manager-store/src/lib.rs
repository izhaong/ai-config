...
//! ## 默认路径(PRD §6.1 本机状态)
//!
//! - macOS:`~/Library/Application Support/agents-manager/store.sqlite`
//! - Linux:`~/.config/agents-manager/store.sqlite`(`$XDG_CONFIG_HOME` 优先)
//! - Windows:`%APPDATA%\agents-manager\store.sqlite`
//!
...
// `ProjectRepo` 重导出到顶级,使用方写 `use agents_manager_store::ProjectRepo;` 即可。
...
impl Store {
    ...
    pub fn open_at(path: &Path) -> Result<Self, StoreError> {
        ...
        store.migrate()?;
        tracing::info!(path = %store.path, "agents-manager store 已就绪");
        Ok(store)
    }
...
mod tests {
    ...
    fn default_path_is_under_data_dir_or_home() {
        ...
        assert!(p.ends_with("agents-manager/store.sqlite") || p.ends_with("agents-manager\\store.sqlite"));
    }
