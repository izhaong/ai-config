//! `ai-config daemon ...` 子命令(PRD §5.1 + §9.1)。
//!
//! 守护进程管控:`start` / `stop` / `status` / `logs` / `events-follow`。
//! 设计要点(对齐 PRD §5.1 / §8.1 / §9.1 / §10 A-5):
//!
//! - **pid file**:`$XDG_CONFIG_HOME/ai-config/daemon.pid`(fallback `~/.config/ai-config/daemon.pid`)
//! - **log file**:`$XDG_CONFIG_HOME/ai-config/daemon.log`
//! - **UDS sock**:`$XDG_CONFIG_HOME/ai-config/daemon.sock`
...
//!
//! 本阶段 `ai-configd` 是 Phase 0 占位,可能不接 UDS — `events-follow` 在
...
fn config_dir() -> PathBuf {
    ...
    if let Some(home) = std::env::var_os("HOME") {
        return PathBuf::from(home).join(".config").join("ai-config");
    }
    PathBuf::from(".config").join("ai-config")
}
...
/// `daemon start` — fork 子进程跑 ai-configd;写 pid file;二次 start 拒绝(退出码 5)。
    ...
    if let Some(pid) = read_pid() {
        if pid_alive(pid) {
            emit_error_envelope(
                ...
                Some(&format!(
                    "pid file at {} holds live pid {pid};stop it first(`ai-config daemon stop`)",
                    pid_file().display()
                )),
            );
    ...
    let exe = match locate_daemon_binary() {
        ...
        None => {
            emit_error_envelope(
                mode,
                exit_code::FS_ERROR,
                "ai-configd binary not found",
                Some(
                    "looked next to current exe and in $PATH;install ai-configd or run `cargo build -p ai-config-daemon`",
                ),
            );
            return ExitCode::from(exit_code::FS_ERROR);
        }
    ...
    let pid = match read_pid() {
        ...
        _ => {
            emit_error_envelope(
                ...
                "daemon not running",
                Some(&format!(
                    "sock path: {};start daemon with `ai-config daemon start`",
                    sock_file().display()
                )),
            );
            return ExitCode::from(exit_code::FS_ERROR);
...
/// 找 `ai-configd` 二进制:优先当前 exe 同目录(开发/打包),再 `$PATH`。
fn locate_daemon_binary() -> Option<PathBuf> {
    let candidate_names = ["ai-configd", "ai-configd.exe"];
