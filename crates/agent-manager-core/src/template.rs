...
pub const MANAGED_BY_VALUE: &str = "agents-manager";
...
///    (不写盘;不留空字符串 — PRD §2.1)
/// 4. 合并已有 mcp.json(如含 `managedBy == "agents-manager"`)
...
pub fn render_mcp(
    ...
    if !missing.is_empty() {
        ...
        hint.push_str("写完后重跑 `agents-manager apply`");
    ...
    // 4. 合并已有(若 managedBy == "agents-manager")
...
/// - 文件存在,顶层含 `"managedBy": "agents-manager"` → 合并;`mcpServers` 字段整块
///   用新的替换,`mcpServers` 之外的字段保留
/// - 文件存在,**不**含 `managedBy == "agents-manager"` → 返回
///   `CoreError::Unmanaged`,**不**写盘(PRD §8.1 硬约束 #5)
pub fn merge_with_existing(dest: &Utf8Path, new_servers: Value) -> Result<Value, CoreError> {
    ...
    // `{ "mcpServers": {...}, "managedBy": "agents-manager" }`
...
fn unmanaged_err(path: &Utf8Path, reason: &str) -> CoreError {
    CoreError::TemplateRender {
        ...
        hint: format!(
            "二选一:(a) `mv {path} {path}.bak` 后重跑;(b) 在该 mcp.json 顶层加 `\"managedBy\": \"agents-manager\"` 表示授权本工具接管"
        ),
    }
}
...
mod tests {
    ...
    fn merge_with_existing_managed_file_replaces_mcp_servers_block_and_preserves_others() {
        ...
        // managedBy 保留
        assert_eq!(obj["managedBy"], "agents-manager");
    }
