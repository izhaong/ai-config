        ...
        Ok(raw) => {
            let v = serde_yaml::from_str(&raw).map_err(|e| {
                yaml_err(
                    ...
                    "修复 YAML 或从 config.yaml.agents-manager.bak.* 恢复",
                )
            })?;
            Ok(Some(v))
        }
...
fn backup_config_if_exists(path: &Utf8Path) -> Result<(), CoreError> {
    ...
    let ts = std::time::SystemTime::now()
        ...
        .map(|d| d.as_secs())
        .unwrap_or(0);
    let bak = path.with_extension(format!("yaml.agents-manager.bak.{ts}"));
...
pub fn migrate_legacy_hermes_mcp_json(home: &Utf8Path) -> Result<HermesMigrateReport, CoreError> {
    ...
    if !report.merged.is_empty() || legacy.is_file() {
        let ts = std::time::SystemTime::now()
            ...
            .map(|d| d.as_secs())
            .unwrap_or(0);
        let bak = legacy.with_extension(format!("json.agents-manager.bak.{ts}"));
