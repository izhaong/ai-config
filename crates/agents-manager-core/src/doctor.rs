//! `agents-manager doctor` 诊断逻辑（GUI / CLI 共用）。
...
/// 将各 IDE 平台目录中仍指向 agents-manager 源的 **symlink / 同 inode** 下发迁移为实体硬拷贝。
pub fn materialize_legacy_symlink_deploys(
    ...
    repaired.extend(materialize_orphan_symlinks_under_agents_manager(default_root)?);
    Ok(repaired)
}
...
fn materialize_orphan_symlinks_under_agents_manager(
    default_root: &Utf8Path,
) -> Result<Vec<String>, CoreError> {
    let mut repaired = Vec::new();
