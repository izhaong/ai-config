...
struct AgentsManagerAdapter {
    asset_root: Utf8PathBuf,
}

impl PlatformAdapter for AgentsManagerAdapter {
    fn id(&self) -> PlatformId {
...
pub fn agentsmanager_adapter(asset_root: &camino::Utf8Path) -> Box<dyn PlatformAdapter> {
    Box::new(AgentsManagerAdapter {
...
pub fn for_scope_with_asset(
    ...
    if id == PlatformId::AgentsManager {
        return Ok(agentsmanager_adapter(asset_root));
    }
...
pub fn kind_asset_path(
    ...
    match kind {
        ...
        AssetKind::Hook => Some(match plat {
            ...
            PlatformId::AgentsManager => asset_root.join("hooks"),
        }),
    }
}
...
pub fn for_scope(
    ...
    if deploy_base == home() {
        return match id {
            PlatformId::AgentsManager => unreachable!(),
...
pub fn parse_platform_str(s: &str) -> Result<PlatformId, CoreError> {
    match s {
        ...
        other => Err(CoreError::InvalidPath(format!(
            "未知平台 `{other}`(预期 agentsmanager/cursor/codex/claude/hermes)"
        ))),
    }
}

/// 解析 deploy / retract 目标平台（排除 agentsmanager 源）。
