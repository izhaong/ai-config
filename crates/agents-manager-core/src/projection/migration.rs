...
fn hook_name_from_platform_command(
    ...
    if path.is_absolute() {
        let relative_root = match platform {
            ...
            PlatformId::AgentsManager => return None,
        };
...
fn is_excluded_agent_entry(name: &str) -> bool {
    ...
    name.starts_with('.')
        ...
        || lowercase.ends_with(".agents-manager-deploy.json")
}
...
fn classify_regular_asset(
    ...
    if accepts_legacy_marker && is_regular_file(&path.join(".agents-manager-deploy.json"))? {
        return Ok((
...
mod tests {
    ...
    fn legacy_marker_is_only_a_candidate_not_ownership() {
        ...
        write(
            &request
                .deploy_base
                .join(".cursor/skills/demo/.agents-manager-deploy.json"),
    ...
    fn project_rule_uses_effective_project_layer_and_keeps_global_raw_source() {
        let temp = TempDir::new().unwrap();
        let root = Utf8Path::from_path(temp.path()).unwrap();
        let global = root.join("global/.agents-manager");
        let project = root.join("project/.agents-manager");
        let request = InventoryRequest {
    ...
    fn hook_halves_split_across_layers_are_both_cross_layer_conflicts() {
        ...
        let root = Utf8Path::from_path(temp.path()).unwrap();
        let global = root.join("home/.agents-manager");
        let workspace = root.join("workspace/.agents-manager");
        let request = InventoryRequest {
            canonical_layers: vec![
                CanonicalLayerRoot {
                    layer: SourceLayer::Global,
