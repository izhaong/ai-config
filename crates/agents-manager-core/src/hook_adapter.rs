...
pub fn platform_hooks_dir(deploy_base: &Utf8Path, plat: PlatformId) -> Utf8PathBuf {
    match plat {
        ...
        PlatformId::AgentsManager => hook::hooks_dir(deploy_base),
    }
}
...
fn platform_config_path(deploy_base: &Utf8Path, plat: PlatformId) -> Utf8PathBuf {
    match plat {
        ...
        PlatformId::AgentsManager => Utf8PathBuf::new(),
    }
}
...
pub fn is_deployed(deploy_base: &Utf8Path, plat: PlatformId, script_filename: &str) -> bool {
    ...
    match plat {
        ...
        PlatformId::AgentsManager => hook::script_path(deploy_base, script_filename).is_file(),
    }
}
...
pub fn read_platform_bindings(
    ...
    match plat {
        PlatformId::AgentsManager => Ok(hook::load_spec(deploy_base, script_filename)?.bindings),
        PlatformId::Cursor | PlatformId::Codex => {
            let path = platform_config_path(deploy_base, plat);
            if !path.is_file() {
                return Ok(Vec::new());
...
fn platform_command(
    ...
    match plat {
        ...
        PlatformId::AgentsManager => source_command.to_string(),
    }
}
