...
pub fn lifecycle_supported(plat: PlatformId, lifecycle: &str) -> bool {
    match plat {
        PlatformId::AgentsManager | PlatformId::Cursor => true,
...
pub fn list_lifecycle_views(
    ...
    let platform = if browse_plat == PlatformId::AgentsManager {
        Vec::new()
    } else {
        platform_bindings(deploy_base, browse_plat, script_filename)
    };
    let active_bindings = if browse_plat == PlatformId::AgentsManager {
        &source
    } else {
        &platform
    };

    let active_plat = if browse_plat == PlatformId::AgentsManager {
        PlatformId::Cursor
    } else {
        browse_plat
    };
    ...
    lifecycle_catalog()
        .iter()
        .filter(|def| {
            browse_plat == PlatformId::AgentsManager
...
mod tests {
    ...
    fn list_views_reflects_source_bindings() {
        ...
        let views = list_lifecycle_views(&root, &root, PlatformId::AgentsManager, "demo.py");
        let st = views
            .iter()
  ...
  "hooks": {{
    ...
    fn toggle_source_lifecycle_updates_manifest() {
        ...
        toggle_lifecycle(&root, &root, PlatformId::AgentsManager, "demo.py", "stop", true).unwrap();

        let views = list_lifecycle_views(&root, &root, PlatformId::AgentsManager, "demo.py");
        let stop = views.iter().find(|v| v.lifecycle == "stop").unwrap();
        assert!(stop.active);

        toggle_lifecycle(&root, &root, PlatformId::AgentsManager, "demo.py", "stop", false).unwrap();
        let views = list_lifecycle_views(&root, &root, PlatformId::AgentsManager, "demo.py");
        let stop = views.iter().find(|v| v.lifecycle == "stop").unwrap();
        assert!(!stop.active);
    }
