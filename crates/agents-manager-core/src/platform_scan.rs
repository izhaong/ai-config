...
pub fn scan_platform_assets(
    ...
    if plat == PlatformId::AgentsManager {
        return scan_agentsmanager_assets(kind, default_root, asset_root);
    }
    ...
    for (name, platform_path, description, hook_type, binding_key) in raw {
        ...
        let source_state = if states.get(&PlatformId::AgentsManager) == Some(&LinkState::Linked) {
            SourceState::Managed
        } else {
...
fn scan_agentsmanager_assets(
    ...
    let adapter = platform::agentsmanager_adapter(asset_root);
    let source_scan = scan_source_for_scope(default_root, asset_root)?;
...
fn browse_platform_link_state(
    ...
    if browse_plat == PlatformId::AgentsManager {
...
mod tests {
    ...
    fn import_skill_copies_to_source() {
        ...
        assert_eq!(states.get(&PlatformId::AgentsManager), Some(&LinkState::Linked));
        assert_eq!(
    ...
    #[test]
    fn agentsmanager_unlinked_when_not_in_source() {
        let tmp = TempDir::new().unwrap();
        let root = Utf8Path::from_path(tmp.path()).unwrap();
        let states = compute_entry_states(
            PlatformId::Claude,
        ...
        assert_eq!(
            states.get(&PlatformId::AgentsManager),
    ...
    #[test]
    fn agentsmanager_linked_when_exists_in_source_on_ide_view() {
        let tmp = TempDir::new().unwrap();
        let home = Utf8Path::from_path(tmp.path()).unwrap();
        let _home_guard = crate::test_env::EnvGuard::set("HOME", tmp.path().to_str().unwrap());
