...
/// 收回平台 MCP（Hermes 仅移除 `mcp_servers` 中 agents-manager 管理的条目，见 per-server retract）。
...
/// 收回平台 MCP 整文件；若与 `source_mcp` 同路径则 noop，避免误删 agents-manager 源。
...
/// 平台 `mcp.json` 是否与 agents-manager 源共用同一文件（symlink / 硬链接 / 同路径）。
...
mod tests {
    ...
    #[test]
    fn upsert_gui_style_agents_manager_server() {
        let tmp = TempDir::new().unwrap();
        let root = Utf8PathBuf::from_path_buf(tmp.path().to_path_buf()).unwrap();
        ...
        upsert_server_in_document(&root, "agents-manager", cfg.clone()).unwrap();
        let stored = get_server_config(&root, "agents-manager")
            .unwrap()
            .expect("stored");
        assert_eq!(stored["command"].as_str(), Some("agents-manager"));
    ...
    fn hermes_upsert_writes_config_yaml() {
        ...
        let root = home.join("agents-manager-asset");
    ...
    /// 平台 mcp.json 若 symlink 到 agents-manager 源，retract 单条 server 不得改动源文件。
    #[test]
    #[cfg(unix)]
