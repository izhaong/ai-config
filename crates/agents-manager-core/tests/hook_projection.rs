...
use agents_manager_core::projection::executor::{apply_projection_plan, ApplyOptions, ExecutorContext};
...
use agents_manager_core::projection::source::{resolve_effective_assets, OverlayRoots};
use agents_manager_core::source::scan_project_root;
use camino::Utf8Path;
use chrono::Utc;
...
) -> Result<agents_manager_core::projection::executor::ApplyReport, String> {
...
fn hook_apply_failure_restores_config_and_script_link_together() {
    ...
    impl ProjectionLedger for FailingLedger {
        ...
        ) -> Result<Option<ProjectionRecord>, agents_manager_core::error::CoreError> {
            Ok(None)
        }
        ...
        ) -> Result<Vec<ProjectionRecord>, agents_manager_core::error::CoreError> {
            Ok(Vec::new())
        }
        ...
        ) -> Result<(), agents_manager_core::error::CoreError> {
            Err(agents_manager_core::error::CoreError::ProjectionLedger(
...
fn hook_retract_removes_only_ledger_owned_binding_and_link() {
    ...
    fs::write(
        config.as_std_path(),
        r#"{"foreign":"keep","hooks":{"afterShellExecution":[{"command":"./hooks/foreign.sh"},{"command":".cursor/hooks/owned.sh","managedBy":"agents-manager","hook":"owned.sh"}]}}"#,
    )
    .unwrap();
