...
use agents_manager_core::projection::ledger::MemoryProjectionLedger;
use agents_manager_core::projection::model::{DeploymentScope, SourceLayer};
...
fn project_template_seeds_the_canonical_agents_prompt() {
    let prompt = project_template_root().join(".agents/prompts/AGENTS.md");

    assert!(
        prompt.is_file(),
        "the project template must seed the complete prompt at .agents/prompts/AGENTS.md"
    );
    assert!(
...
fn project_template_root_agents_is_an_exact_canonical_prompt_link() {
    ...
    assert_eq!(
        ...
        std::path::PathBuf::from(".agents/prompts/AGENTS.md"),
...
fn prompt_plans_one_project_entry_for_all_platforms() {
    ...
    write(
        &root.join("project/.agents/prompts/AGENTS.md"),
...
fn prompt_overlay_then_reapply_is_an_unchanged_noop() {
    ...
    let root = Utf8Path::from_path(temp.path()).unwrap();
    write(
        &root.join("global/.agents/prompts/AGENTS.md"),
        "global entry\n",
    );
    write(
        &root.join("project/.agents/prompts/AGENTS.md"),
