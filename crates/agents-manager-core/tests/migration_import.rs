...
use agents_manager_core::projection::fingerprint::path_fingerprint;
...
fn prompt_request(root: &Utf8Path, replace: bool) -> ImportRequest {
    ImportRequest {
        ...
        destination_asset_root: root.join("repo/.agents-manager"),
        replace,
    }
}
...
fn scope_import_request_uses_only_the_current_platform_target_and_canonical_layer() {
    ...
    let roots = SyncRoots {
        ...
        asset_root: root.join("repo/.agents-manager"),
        global_default: root.join("global/.agents-manager"),
    ...
    assert_eq!(request.destination_asset_root, root.join("repo/.agents-manager"));
}
...
fn second_action_validation_failure_rolls_back_prior_and_current_writes() {
    ...
    let destination_asset_root = root.join("repo/.agents-manager");
...
fn rollback_refuses_a_concurrent_import_lock_without_changing_the_canonical_source() {
    ...
    .unwrap();
    write(
        &transaction_root.join(".agents-manager-import.lock"),
        "concurrent apply\n",
    );
...
fn rollback_recovers_a_write_completed_before_the_applied_flag_was_persisted() {
    ...
    .unwrap();
    let approved = root.join("repo/.agents-manager");
