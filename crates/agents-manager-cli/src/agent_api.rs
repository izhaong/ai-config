...
use agents_manager_core::asset_ops::{self, ScopeRoots};
...
/// 资产根扫描摘要（MCP `agents_manager_env`）。
...
pub fn parse_platform(s: &str) -> Result<PlatformId, String> {
    match s.to_lowercase().as_str() {
        ...
        "agentsmanager" | "agents-manager" => Ok(PlatformId::AgentsManager),
        other => Err(format!(
            "未知平台 `{other}`；可用: cursor, codex, claude, hermes, agentsmanager"
        )),
    }
}
...
pub fn deploy(
    ...
    Err(CoreError::NotImplemented(
        "MCP 单项 deploy 尚未映射到 source-first projection plan；请使用 agents_manager_sync 并显式 apply=true",
    ))
}
...
pub fn retract(
    ...
    Err(CoreError::NotImplemented(
        "MCP 单项 retract 尚未映射到 source-first projection plan；请使用 agents_manager_sync 的 uninstall 生命周期",
    ))
}
...
mod tests {
    ...
    fn default_scope_resolution_does_not_initialize_asset_root() {
        ...
        let previous_root = std::env::var("AGENTS_MANAGER_ROOT").ok();
        std::env::set_var("HOME", &home);
        std::env::remove_var("AGENTS_MANAGER_ROOT");
        ...
        if let Some(value) = previous_root {
            std::env::set_var("AGENTS_MANAGER_ROOT", value);
        } else {
            std::env::remove_var("AGENTS_MANAGER_ROOT");
        }
    }
