...
use agents_manager_core::paths::{self, SyncRoots};
...
use agents_manager_core::projection::mcp::source::resolve_effective_mcp_definitions;
...
use agents_manager_core::secrets;
...
pub(crate) fn rollback_reviewed_migration(
    ...
    let context = ExecutorContext::new(
        ...
        roots.deploy_base.join(".agents-manager/projection-backups"),
    );
...
fn execute_workspace(
    ...
    let ledger_path = workspace_root.join(".agents-manager/projection-ledger.sqlite");
    let secrets = LifecycleMcpSecrets::load()?;
...
fn build_workspace_bundles(
    ...
    agents_manager_core::workspace::discover_members(workspace_root)?
        .into_iter()
...
fn apply_workspace_bundles(
    ...
    let plan_members = bundles
        ...
        .collect::<Vec<_>>();
    let backup_root = workspace_root.join(".agents-manager/projection-backups");
...
fn is_missing_secret_skip(action: &agents_manager_core::projection::planner::ProjectionAction) -> bool {
