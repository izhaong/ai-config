...
pub fn emit_json<T: serde::Serialize>(mode: OutputMode, value: &T) {
    if mode.is_json() {
        match serde_json::to_string_pretty(value) {
            Ok(s) => println!("{s}"),
            Err(e) => eprintln!("[agents-manager] serialize error: {e}"),
        }
    }
}
...
pub fn report_anyhow(mode: OutputMode, err: anyhow::Error) -> ExitCode {
    if let Some(core) = err.downcast_ref::<agents_manager_core::error::CoreError>() {
        let code = core.exit_code();
        emit_error_envelope(mode, code, &core.to_string(), core.hint());
        return ExitCode::from(code);
    }
    let code = agents_manager_core::error::exit_code::FS_ERROR;
