...
use agents_manager_core::doctor::DoctorReport;
...
#[derive(Debug, Clone)]
pub struct AgentsManagerMcpServer {
    default_root: Arc<Utf8PathBuf>,
    tool_router: ToolRouter<Self>,
}

impl AgentsManagerMcpServer {
...
impl AgentsManagerMcpServer {
    ...
    async fn agents_manager_status(
        &self,
        Parameters(params): Parameters<RootParam>,
    ...
    async fn agents_manager_doctor(
        &self,
        Parameters(params): Parameters<RootParam>,
    ...
    async fn agents_manager_deploy(
        &self,
    ...
    async fn agents_manager_retract(
        &self,
        Parameters(params): Parameters<DeployParam>,
    ) -> Result<Json<OkMessage>, McpError> {
        let kind = parse_kind(&params.kind).map_err(Self::err_msg)?;
...
#[tool_handler(
    ...
    name = "agents-manager",
    instructions = "Manage agents-manager assets across Cursor, Codex, Claude Code, and Hermes. Prefer agents_manager_doctor before sync/deploy. Never expose secrets values."
)]
impl ServerHandler for AgentsManagerMcpServer {
...
pub fn run(default_root: Utf8PathBuf) -> anyhow::Result<()> {
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "agents_manager_cli=info,rmcp=warn".into()),
        )
    ...
    rt.block_on(async {
        let server = AgentsManagerMcpServer::new(default_root);
...
mod tests {
    ...
    fn tool_output_schemas_are_objects() {
        let server = AgentsManagerMcpServer::new(paths::discover_global_asset_root());
    ...
    fn mutation_tools_require_explicit_apply_and_sync_returns_projection_plan() {
        let server = AgentsManagerMcpServer::new(paths::discover_global_asset_root());
        let tools = server.tool_router.list_all();
        for name in ["agents_manager_sync", "agents_manager_deploy", "agents_manager_retract"] {
            let tool = tools
                .iter()
        ...
        let sync = tools
            .iter()
            .find(|tool| tool.name == "agents_manager_sync")
            .expect("sync tool");
    ...
    async fn sync_handler_is_plan_only_by_default_and_foreign_guard_blocks_apply() {
        ...
        let server = AgentsManagerMcpServer::new(root.clone());

        let plan_only = server
            .agents_manager_sync(Parameters(RootParam {
                root: None,
                apply: false,
            }))
            .await
        ...
        for operation in ["deploy", "retract"] {
            ...
            let result = if operation == "deploy" {
                server.agents_manager_deploy(Parameters(params)).await
            } else {
                server.agents_manager_retract(Parameters(params)).await
            };
    ...
    async fn sync_handler_missing_mcp_secret_returns_a_safe_partial_report_and_applies_direct_assets(
        ...
        let _asset_root = EnvVarGuard::remove("AGENTS_MANAGER_ROOT");
  ...
  "config": {{
    ...
    "env": {{ "{MISSING_SECRET_KEY}": "${{{MISSING_SECRET_KEY}}}" }}
        ...
        let server = AgentsManagerMcpServer::new(root);
