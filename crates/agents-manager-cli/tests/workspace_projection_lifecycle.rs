...
impl WorkspaceFixture {
    ...
    fn configure_three_layer_overlay(&self) {
        for member in ["one", "two"] {
            let assets = self.member(member).join(".agents");
        ...
        write(
            &self
                .home
                .path()
                .join(".agents/skills/global-default/SKILL.md"),
            "---\nname: global default\n---\nglobal default\n",
        );
        write(
            &self
        ...
        write(
            &self
                .workspace
                .path()
                .join(".agents/skills/workspace-default/SKILL.md"),
            "---\nname: workspace default\n---\nworkspace default\n",
        );
        write(
            &self.member("one").join(".agents/skills/shared/SKILL.md"),
            "---\nname: project shared\n---\nproject shared\n",
        );
    }
}
...
fn workspace_plan_overlays_workspace_defaults_for_local_and_empty_members_without_home_targets() {
    ...
    assert_source_and_member_target(
        ...
        "shared",
        "project",
        &fixture.member("one").join(".agents/skills/shared"),
    );
    assert_source_and_member_target(
        one,
        "one",
        ...
        &fixture
            .workspace
            .path()
            .join(".agents/skills/workspace-default"),
    );
    assert_source_and_member_target(
        one,
        "one",
        "global-default",
        "global",
        &fixture.home.path().join(".agents/skills/global-default"),
    );
    ...
    assert_source_and_member_target(
        ...
        "shared",
        "workspace",
        &fixture.workspace.path().join(".agents/skills/shared"),
    );
    assert_source_and_member_target(
        two,
        "two",
...
fn workspace_apply_uses_three_layer_overlay_for_each_member_without_writing_home() {
    ...
    let workspace_assets = fixture.workspace.path().join(".agents/skills");
    let global_assets = fixture.home.path().join(".agents/skills");
...
fn workspace_runtime_failure_reports_rollback_and_not_applied_members_with_exit_three() {
    ...
    for index in 0..256 {
        write(
            &fixture
                .member("one")
                .join(format!(".agents/skills/{index:04}-bulk/SKILL.md")),
            "---\nname: bulk\n---\ncanonical bulk skill\n",
        );
    }
    let first_target = fixture.member("one").join(".agents/skills/0000-bulk");
    let second_member_source = fixture.member("two").join(".agents/skills/two/SKILL.md");
