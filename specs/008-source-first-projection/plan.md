...
**Goal:** 将 agents-manager 重构为 skills、rules、commands、agents、入口提示词、hooks 与 MCP 的唯一事实源，并以“同格式逐项链接、异构格式安全生成”的单向投影替换五平台对等硬拷贝。
...
- MCP canonical source 必须是 `mcp/servers/<name>.json` + secret references；真实 secret 只在 `~/.config/agents-manager/secrets.env`，权限 0600。
...
| 根因 | 当前位置 | 影响 |
| --- | --- | --- |
| 强制硬拷贝、无事务覆盖 | `crates/agents-manager-core/src/materialize.rs::deploy` | 产生多份事实、失败后可能留下半份目标 |
...
| link helper 直接替换普通目标 | `crates/agents-manager-core/src/link.rs::link/remove_dest_path` | 错源或 foreign 可被覆盖 |
| status/doctor/list 扫描有副作用 | `source.rs::scan_project_root`、`paths.rs::discover_global_asset_root` | 声称只读但可能迁移/播种/写 hook |
| 项目 MCP 丢失 deploy_base | `crates/agents-manager-cli/src/lifecycle.rs::execute_all_actions` | project sync 可写到 HOME |
...
| store/daemon 未完成却进入产品语义 | `agents-manager-store/src/schema.rs`、daemon/bus/watcher | 增加状态源但不能提供可靠所有权 |
...
<workspace>/.agents-manager (optional workspace defaults)
            +
<repo>/.agents-manager (project override)
            │
            ▼
...
| 主要 Crate | `agents-manager-core`、`agents-manager-store`、`agents-manager-cli`、`agents-manager-gui` |
...
- [x] 业务判断、规划和 apply 逻辑仅在 `agents-manager-core`。
...
```text
crates/agents-manager-core/src/projection/
...
```text
crates/agents-manager-core/src/mcp/
├── mod.rs
...
| repo | `.agents-manager/*`, `.gitignore`, generated platform dirs | Migrate/Delete | 本仓 dogfood canonical source，去除实体副本 |
...
SQLite `projections` 表由 `agents-manager-store` 实现，但 core 只依赖 `ProjectionLedger` trait；core 测试使用 `MemoryProjectionLedger`。读失败时 planner 安全降级为“不证明 generated ownership”；apply 阶段 `apply_batch` 必须在一个 SQLite transaction 内提交全部 upsert/remove，任何写失败都视为 transaction failure 并触发文件系统回滚，不能静默降级。
...
1. 获取 `~/.config/agents-manager/apply.lock` 排他锁。
2. 验证 plan schema、digest、每条 source/target precondition。
3. 在 `~/.config/agents-manager/backups/<transaction-id>/` 建立 0700 transaction 目录。
...
- Cursor/Claude 使用 JSON object-level merge，保留非 agents-manager server 和顶层字段。
...
- agents-manager source icon 永远只读，不再可“retract source”。
...
- 删除平台到平台 copy；平台视图唯一反向操作为“Import to agents-manager”。
...
- 每次 apply 都生成 transaction backup + manifest；`agents-manager migrate rollback <transaction-id>` 仅在 post-apply fingerprint 未漂移时自动恢复。
...
- Modify: `crates/agents-manager-core/src/materialize.rs`
- Modify: `crates/agents-manager-core/src/link.rs`
- Modify: `crates/agents-manager-core/src/asset_ops.rs`
- Modify: `crates/agents-manager-core/src/asset_scope.rs`
- Modify: `crates/agents-manager-core/src/mcp_json.rs`
- Modify: `crates/agents-manager-core/src/source.rs`
- Modify: `crates/agents-manager-core/src/paths.rs`
- Modify: `crates/agents-manager-core/src/doctor.rs`
- Modify: `crates/agents-manager-cli/src/lifecycle.rs`
- Test: `crates/agents-manager-cli/tests/deploy_retract.rs`
...
- [x] **T001.2 Verify RED**：运行 `cargo test -p agents-manager-core retract_refuses_unowned -- --nocapture` 与 `cargo test -p agents-manager-cli --test deploy_retract`；预期至少上述新测试因普通目标被删除、MCP 整文件删除或 scope 错误而失败，且测试过滤结果必须显示至少 1 个用例执行。
...
- [x] **T001.6 Verify GREEN**：运行 `cargo test -p agents-manager-core -p agents-manager-cli`；预期全部通过，且 `git status --short` 只包含本任务源码/测试变更。
- [x] **T001.7 Commit checkpoint**：`git commit -m "fix(core): 封死未托管资产误删与只读写入"`。
- [x] **T001.8 Audit regression closure**：补充并先验证 RED：删除 source 时保留未托管普通目录、第三方软链接和无 ownership 的同名平台 MCP；`materialize::retract` 无 expected source 时拒绝任何软链接，只有 canonical-equivalent 的精确 legacy symlink 可收回。验证：`cargo test -p agents-manager-core source_delete_preserves_ -- --nocapture`、`cargo test -p agents-manager-core retract_ -- --nocapture`、`cargo test -p agents-manager-core -p agents-manager-cli -- --test-threads=1`。
...
- [x] **T001.10 Hook ownership regression closure**：先复现 Cursor 外部 Hook 仅有第三方 binding 时，`hook_adapter::retract` 仍会删除脚本；现在未发现 `agents-manager` managed binding 就整体跳过，配置与脚本都保持不变。验证：`retract_preserves_external_hook_script_without_managed_binding` RED→GREEN，随后 `cargo test -p agents-manager-core -p agents-manager-cli` 全绿。

### T002 — 统一 projection model、fingerprint 与 optional ledger

**Files:**

- Create: `crates/agents-manager-core/src/projection/mod.rs`
- Create: `crates/agents-manager-core/src/projection/model.rs`
- Create: `crates/agents-manager-core/src/projection/fingerprint.rs`
- Create: `crates/agents-manager-core/src/projection/ownership.rs`
- Create: `crates/agents-manager-store/src/projection_repo.rs`
- Modify: `crates/agents-manager-core/src/lib.rs`, `model.rs`, `error.rs`
- Modify: `crates/agents-manager-store/src/lib.rs`, `schema.rs`
- Modify: `Cargo.toml`, `crates/agents-manager-core/Cargo.toml` (`sha2 = "0.10"`, `hex = "0.4"`)
...
- [ ] **T002.2 Verify RED**：`cargo test -p agents-manager-core projection:: -- --nocapture`，预期因模块/类型不存在失败。
...
- [ ] **T002.6 Verify**：`cargo test -p agents-manager-core projection::`、`cargo test -p agents-manager-store projection_repo`。
...
- Refactor: `crates/agents-manager-core/src/platform.rs` into `src/platform/{mod,cursor,codex,claude,hermes}.rs`
...
- [ ] **T003.2 Verify RED**：`cargo test -p agents-manager-core platform::`；预期 Codex/Claude 当前旧路径断言失败。
...
- [ ] **T003.5 Verify**：`cargo test -p agents-manager-core platform::`。
...
- Create: `crates/agents-manager-core/src/projection/source.rs`
- Modify: `crates/agents-manager-core/src/source.rs`, `paths.rs`, `workspace.rs`
- Test: `crates/agents-manager-core/tests/source_overlay.rs`
...
- [x] **T004.6 Verify**：`cargo test -p agents-manager-core projection::source::tests`（4 passed）与 `cargo test -p agents-manager-core projection::`（63 passed）。
...
- Create: `crates/agents-manager-core/src/projection/planner.rs`
- Modify: `crates/agents-manager-core/src/projection/mod.rs`
- Compatibility modify: `crates/agents-manager-core/src/sync.rs`
- Test: `crates/agents-manager-core/tests/projection_plan.rs`
...
- [x] **T005.6 Verify**：`cargo test -p agents-manager-core --test projection_plan`（20 passed）；release 模式的 1,000 项 × 100 次 p95 < 2 秒门禁通过。
...
- Create: `crates/agents-manager-core/src/projection/executor.rs`
- Refactor: `crates/agents-manager-core/src/link.rs`
- Modify: `crates/agents-manager-core/src/asset_ops.rs`, `asset_scope.rs`, `skills_add.rs`
- Test: `crates/agents-manager-core/tests/projection_apply.rs`
...
- [x] **T006.7 Verify**：`cargo test -p agents-manager-core --test projection_apply`（25 passed），`projection_plan`（20 passed）与 `projection::platform_adapter`（42 passed）。
...
- Create: `crates/agents-manager-core/src/mcp/{mod,source,adapter,cursor_json,codex_toml,claude_json,hermes_yaml}.rs`
- Modify: workspace/core `Cargo.toml` (`toml_edit = "0.25"`, `yaml-edit = "0.2.3"`)
- Modify: `crates/agents-manager-core/src/model.rs`, `secrets.rs`, `projection/planner.rs`, `projection/executor.rs`, `platform/*`
- Modify: `crates/agents-manager-cli/src/mcp.rs`
- Test: `crates/agents-manager-core/tests/mcp_projection.rs`
- Fixtures: `crates/agents-manager-core/tests/fixtures/mcp/*`
...
- [x] **T007.3 Verify RED**：`cargo test -p agents-manager-core --test mcp_projection`。
...
- [x] **T007.8 Verify**：`mcp_projection` 35 passed；CLI `mcp_migrate`、`deploy_retract`、`mcp_plan_apply`、`mcp_source_readonly` 共 31 passed；另 `cargo test -p agents-manager-core -p agents-manager-cli` 全绿。
...
- Modify: `crates/agents-manager-core/src/hook.rs`, `hook_adapter.rs`, `source.rs`, `projection/planner.rs`, `projection/executor.rs`
...
### T009 — CLI 与 agents-manager MCP API 统一走 plan/apply

**Files:**

- Create: `crates/agents-manager-cli/src/projection.rs`
- Create: `crates/agents-manager-cli/tests/projection_lifecycle.rs`
...
- Create: `crates/agents-manager-core/src/projection/migration.rs`
- Create: `crates/agents-manager-cli/src/migration.rs`
- Create: `crates/agents-manager-cli/tests/projection_migration.rs`
- Modify: `asset_ops.rs`, `platform_scan.rs`, `main.rs`, `serve.rs`

**CLI contract:**

```text
agents-manager migrate inventory --json
agents-manager migrate source-first --plan <file> --select <action-id>... [--extract-secrets] [--apply]
agents-manager migrate rollback <transaction-id>
agents-manager import <kind> <name> --from <platform> --to global|project [--replace] [--apply]
agents-manager adopt --plan <file> --select <action-id>... --apply
```
...
即使成员没有本地 `.agents-manager` 也继承 workspace/global defaults；apply 仅写各成员 deploy target，
...
`agents-manager mcp migrate <container> --extract-secrets --apply` 在没有 reviewed plan/action ID 时会直接
...
`agents-manager import` / `migrate source-first`。既有正向 legacy 写测试改为更强的零写拒绝合同；最终
...
- [ ] **T010.7 Verify**：`cargo test -p agents-manager-cli --test projection_migration` 与 `cargo test -p agents-manager-cli --test projection_migration_actions`。
...
- Modify: `apps/agents-manager-gui/src/command_bridge.rs`, `src/lib.rs`
...
- [ ] **T011.1 Write failing Rust bridge tests**：Tauri bridge 只调用 core；默认 plan-only；无 `materialize`/platform-to-platform implementation；planner ledger read error 安全降级为“不证明 ownership”，apply ledger write error 则中止并回滚；同一规范化 fixture 经 CLI、agents-manager MCP API、Tauri bridge 的 schema/action ordering/plan digest 完全一致。
- [ ] **T011.2 Write failing frontend state tests**：Managed 才可 retract；Missing 可 create；Equivalent 需基于当前 plan digest + action_id adopt confirmation；不同内容 Foreign 不可直接 adopt，只可 import；Conflict/Drifted 只可 diff/import/repair；agents-manager icon 不可 retract；无 peer/bulk takeover。
- [ ] **T011.3 Verify RED**：`cargo test -p agents-manager-gui`；`cd apps/agents-manager-gui && npm run test -- src/utils/entryPlatformToggle.test.ts src/components/assets/PlatformIconButtons.test.tsx`。
...
- [ ] **T011.6 Remove peer-copy UX**：平台视图仅 Import to agents-manager；source view 才可投影；列表状态文案不再把 synced/linked 混用。
- [ ] **T011.7 Verify**：`cargo test -p agents-manager-gui`；`cd apps/agents-manager-gui && npm run test && npm run build`。
...
- Move canonical project assets into `.agents-manager/{skills,rules,agents,commands,prompts,hooks,mcp}`
...
- Modify: `.specify/memory/constitution.md`, `.cursor/rules/00-agents-manager-core.mdc`, `hooks-asset-layout.mdc`
...
- [ ] **T012.7 Verify hygiene**：`cargo test -p agents-manager-cli --test repository_hygiene`。
...
- [ ] **T013.2 Core/CLI/store tests**：`cargo test -p agents-manager-core -p agents-manager-store -p agents-manager-cli -- --test-threads=1`；预期 0 failed。
- [ ] **T013.3 Workspace tests**：`cargo test --workspace -- --test-threads=1`；预期 0 failed。
- [ ] **T013.4 GUI tests/build**：`cd apps/agents-manager-gui && npm run test && npm run build`；随后仓库根 `cargo test -p agents-manager-gui`；预期全绿。
...
- [ ] **T013.7 Real-HOME inventory-only**：`./target/debug/agents-manager migrate inventory --json`、`sync --json`、`doctor --json`、`status --json`；执行前后 HOME allowlisted roots tree hash 必须完全一致。
