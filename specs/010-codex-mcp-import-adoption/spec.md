...
**Created**: 2026-07-23  
**Status**: Approved  
**Input**: 修复 agents-manager，使项目现有 `.codex/config.toml` 中的 MCP 可无损导入项目 source、在 GUI 显示，并在语义等价时建立受管所有权而不改写目标配置。
...
canonical source 与现有 Codex entry 语义等价时，用户可以显式确认 adopt，agents-manager 只建立逐 server ownership ledger，不重写 `.codex/config.toml`。
...
- **SC-004**: `cargo test -p agents-manager-core -p agents-manager-cli`、GUI Rust tests、Vitest 和 build 均为退出码 0。
