...
**Created**: 2026-07-23  
**Status**: Approved  
**Input**: 修复 agent-manager，使项目现有 `.codex/config.toml` 中的 MCP 可无损导入项目 source、在 GUI 显示，并在语义等价时建立受管所有权而不改写目标配置。

## User Scenarios & Testing

### User Story 1 - 从 Codex 项目配置导入 MCP (Priority: P1)

用户在任意目录克隆项目后，可以把 `.codex/config.toml` 中指定的 `[mcp_servers.<name>]` 导入 `<repo>/.agent-manager/mcp/servers/<name>.json`，不依赖旧电脑绝对路径，也不写入真实凭据。
...
canonical source 与现有 Codex entry 语义等价时，用户可以显式确认 adopt，agent-manager 只建立逐 server ownership ledger，不重写 `.codex/config.toml`。
...
- **SC-004**: `cargo test -p agent-manager-core -p agent-manager-cli`、GUI Rust tests、Vitest 和 build 均为退出码 0。
