...
.codex/config.toml#mcp_servers.<name>
  ...
  -> .ai-config/mcp/servers/<name>.json
...
| 文件 | 职责 |
| --- | --- |
| `crates/ai-config-core/src/projection/migration_action.rs` | Codex import source path、TOML entry normalization、secret preflight |
| `crates/ai-config-core/src/projection/migration.rs` | Codex MCP inventory/equivalence/adopt action |
| `crates/ai-config-core/tests/migration_import.rs` | import RED/GREEN 与安全边界 |
| `crates/ai-config-cli/tests/projection_migration.rs` | CLI plan/apply/rollback、零目标写入 |
| `apps/ai-config-gui/src/command_bridge.rs` | 复用 core 的 GUI import/adopt 测试 |
| `apps/ai-config-gui/src/**` | Import/Adopt 状态与交互测试，必要的最小 UI 接线 |
| `apps/ai-config-gui/src-tauri/Cargo.toml` | 版本统一为 0.4.0 |
...
- [x] **T001 Core import RED**：在 `migration_import.rs` 添加 Codex stdio、bearer env、env header、literal Authorization、missing entry、drift 测试；运行 `cargo test -p ai-config-core --test migration_import codex_mcp -- --nocapture`，预期因 NotImplemented/缺解析失败。
...
- [x] **T008 全量验证**：`cargo test -p ai-config-core -p ai-config-cli`、`cargo test -p ai-config-gui`、`npm run test`、`npm run build`、debug CLI doctor/status；全部退出码 0，代码审查无剩余 Critical/Important。
