...
`hook.yaml` 为 **agents-manager 唯一 canonical 格式**（事件、matcher、入口脚本相对路径）；下发时由 adapter 转成各平台原生配置，收回时按 `managedBy: agents-manager` + hook `name` 摘除条目并删脚本副本。
...
- **FR-006**: 托管标记：`managedBy: agents-manager` + `hook: <name>`（各平台 manifest 内）
