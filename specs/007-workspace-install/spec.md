...
**Created**: 2026-06-26  
**Status**: Complete  
**Input**: zh-cloud #42 — 子仓 `.codex` 覆盖不足、bootstrap 与 agents-manager 分拆

## 背景

`scripts/bootstrap-repo-agents-manager.sh` 为每个子仓**手工列举** rules/skills/commands/agents/hooks，与 agents-manager 的 `compute_for_project` 模型并行。用户级 `agents-manager sync` 已健康，但 repo 级需重复跑 bootstrap。
...
**Given** bootstrap 仍负责 per-repo 精选列表与 COMMANDS.md，**When** workspace install 落地，**Then** bootstrap 可瘦身为调用 `agents-manager install --workspace` + 保留 zh-cloud 专有模板（hooks 变体、cursorignore）。
...
- **SC-002**: `cargo test -p agents-manager-core -p agents-manager-cli`
