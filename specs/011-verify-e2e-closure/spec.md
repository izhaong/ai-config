...
1. **Given** PATH 中存在 `agents-manager`，**When** 其版本与 `target/debug/agents-manager` 不同，**Then** 验证必须失败并报告版本漂移。
2. **Given** PATH 中不存在 `agents-manager`，**When** 执行源码验证，**Then** 源码测试可继续，但运行态闭环必须标为未验证。
...
**Why this priority**: `agents-manager completion zsh | head` 当前稳定触发 broken-pipe panic 和退出码 101。
...
- PATH 中的 `agents-manager` 是 symlink 或不同安装渠道。
- 临时 HOME 中不存在任何平台父目录。
