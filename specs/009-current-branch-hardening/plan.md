...
范围包括被意外清空的 tracked Hook 配置、GUI 平台垃圾桶的 core 能力对齐、共享列表布局的 E2E 选择器恢复、已存在的 GUI lint 错误，以及 Rust 单测的进程级环境变量隔离。默认平台为 ai-config 和详情抽屉滚动修复保留并回归验证。

## 背景与根因

- `.cursor/hooks.json` 当前仅留下空 `hooks` 对象，删除了 HEAD 中所有 `managedBy: ai-config` 的 Hook 绑定，且产生尾随空格；依据 hooks asset layout，它是平台投影，不能以空白内容替代受管绑定。
...
| Core crate | `crates/ai-config-core` |
...
- [x] 业务所有权语义保留在 `ai-config-core`；GUI 只根据已知 core 能力决定是否启用操作。
- [x] 不新增平台路径或同步规则。
...
| `.cursor/skills/ai-config-delivery/SKILL.md` | 恢复规则 | 恢复详细 Plan 门禁 |
...
恢复 HEAD 中的 tracked `.cursor/hooks.json` 和仓库 delivery skill 详细度文本；不运行 sync、不修改 `~/.ai-config`，避免越过用户资产边界。
...
5. `cargo test -p ai-config-core -p ai-config-cli` 与 `cargo clippy -p ai-config-core -p ai-config-cli -- -D warnings`。
...
- 2026-07-16：`cargo test -p ai-config-core -p ai-config-cli`（0 failures）、`cargo clippy -p ai-config-core -p ai-config-cli -- -D warnings`（exit 0）、`cargo fmt --check`（exit 0）。
