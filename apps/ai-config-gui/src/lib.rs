...
//! | `cmd_list` | `project?` | `AssetList` | `source::scan_with_override` |
...
fn capability_skip_reason(plat: PlatformId, kind: AssetKind) -> String {
    match (plat, kind) {
        ...
        (PlatformId::Codex, AssetKind::Agent) => "Codex 无独立 agents 目录映射".into(),
        ...
        (PlatformId::Hermes, AssetKind::Agent) => {
            "Hermes 无全局 agents 目录；子代理由运行时 delegate 管理".into()
        }
        _ => format!(
...
async fn cmd_list(
    ...
    // 委托给 `resolve_roots` — 与其它 command 走同一路径,避免实现漂移。
    let (default_root, project_root) = resolve_roots(&state, project.as_deref()).await?;

    // 2. 扫
    let scan = source::scan_with_override(&project_root, &default_root)
        .map_err(|e| format!("scan_with_override 失败: {e}"))?;
    ...
    for (kind, name, src) in entries {
        ...
        for plat in [
            ...
            let st = if kind == AssetKind::Mcp {
                let asset_root = src.parent().unwrap_or(&src);
                mcp_link_state(plat, asset_root, &name)
            } else {
                let dest = match compute_dest(plat, kind, &name, &src) {
                    Some(d) => d,
                    None => {
                        states.insert(plat, LinkState::Unlinked);
                        continue;
                ...
                let expected_src = match kind {
                    ...
                    AssetKind::Agent if src.is_dir() => src.clone(),
                    _ => src.clone(),
                };
...
fn mcp_link_state(plat: PlatformId, asset_root: &Utf8Path, server_name: &str) -> LinkState {
    match mcp_json::mcp_server_sync_state_for_platform(asset_root, server_name, plat) {
...
/// Agent 目录形态下发链整个目录。
fn agent_link_src(src: &Utf8Path) -> Utf8PathBuf {
    if src.is_dir() {
        src.to_path_buf()
    } else {
        src.to_path_buf()
...
/// 算一条资产在某个平台的目标路径(与 `core::sync::dest_for` **保持同步**)。
///
/// W9 临时在 GUI 端维护,等 core 把 `sync::dest_for` 改 `pub` 后直接调用。
fn compute_dest(
    plat: PlatformId,
    kind: AssetKind,
    name: &str,
    ...
    let adapter = platform::for_id(plat).ok()?;
    let home_root = match kind {
        AssetKind::Skill => adapter.skills_dir(),
        AssetKind::Rule => adapter.rules_dir(),
        AssetKind::Agent => adapter.agents_dir(),
        AssetKind::Mcp => return None, // MCP 走 template::render_mcp,无单一 dest
    };
...
async fn cmd_skill_deploy(
    ...
    let skill_md = locate_source(&default_root, &project_root, AssetKind::Skill, &name)?;
    let src = skill_link_src(&skill_md);
    let dest = compute_dest(plat, AssetKind::Skill, &name, &skill_md)
...
async fn cmd_skill_delete(
    ...
    let (default_root, project_root) = resolve_roots(&state, project.as_deref()).await?;
    retract_all_platforms_best_effort(&default_root, &project_root, AssetKind::Skill, &name);
    let skill_md = locate_source(&default_root, &project_root, AssetKind::Skill, &name)?;
...
async fn cmd_skill_retract(
    ...
    let src = locate_source(&default_root, &project_root, AssetKind::Skill, &name)?;
    let dest = compute_dest(plat, AssetKind::Skill, &name, &src)
        .ok_or_else(|| format!("无法算 skill `{name}` ← {plat:?} 的 dest"))?;
    let dest_str = dest.to_string();
...
async fn cmd_rule_deploy(
    ...
    let src = locate_source(&default_root, &project_root, AssetKind::Rule, &name)?;
    let dest = compute_dest(plat, AssetKind::Rule, &name, &src)
...
async fn cmd_rule_delete(
    ...
    let (default_root, project_root) = resolve_roots(&state, project.as_deref()).await?;
    retract_all_platforms_best_effort(&default_root, &project_root, AssetKind::Rule, &name);
    let src = locate_source(&default_root, &project_root, AssetKind::Rule, &name)?;
...
async fn cmd_mcp_retract(
    ...
    let (default_root, project_root) = resolve_roots(&state, project.as_deref()).await?;
    let plat = parse_plat(&from)?;
    let _mcp_path = locate_mcp_json(&default_root, &project_root)?;
    let dest_json = platform::for_id(plat)
        .map_err(|e| format!("platform 适配器失败: {e}"))?
        .mcp_json_path();
    let dest_for_blocking = dest_json.clone();
...
async fn cmd_mcp_delete(
    ...
    let (default_root, project_root) = resolve_roots(&state, project.as_deref()).await?;
    retract_all_platforms_best_effort(&default_root, &project_root, AssetKind::Mcp, &name);
    let mcp_path = locate_mcp_json(&default_root, &project_root)?;
    let asset_root = mcp_asset_root(&mcp_path)?;
    let asset_root_for_blocking = asset_root.to_path_buf();
    let name_for_blocking = name.clone();
...
async fn cmd_agent_get(
    ...
    let (default_root, project_root) = resolve_roots(&state, project.as_deref()).await?;
    let src = locate_source(&default_root, &project_root, AssetKind::Agent, &name)?;
...
async fn cmd_agent_delete(
    ...
    let (default_root, project_root) = resolve_roots(&state, project.as_deref()).await?;
    retract_all_platforms_best_effort(&default_root, &project_root, AssetKind::Agent, &name);
    let src = locate_source(&default_root, &project_root, AssetKind::Agent, &name)?;
    let path = src.to_path_buf();
    let path_str = path.to_string();
    tokio::task::spawn_blocking(move || {
...
async fn cmd_agent_deploy(
    ...
    let dest = compute_dest(plat, AssetKind::Agent, &name, &src)
...
async fn resolve_roots(
    ...
    let project_root = match project {
        None | Some("user-global") | Some("") => default_root.clone(),
        Some(name) => {
            // 关键:把 `name`(`&str`,借用自 `state` 之外)复制成 String 闭包内 move。
            // 复制必须发生在 spawn_blocking **之前**,否则 borrow 进 `move ||` 闭包
            // 跨 await 边界(借用检查器会拒)。
            let owned: String = name.to_string();
            let store_for_blocking = state.store.clone();
            tokio::task::spawn_blocking(move || -> Result<Utf8PathBuf, String> {
                store_for_blocking
                    .projects()
                    .get_by_name(&owned)
                    .map_err(|e| e.to_string())?
...
fn retract_all_platforms_best_effort(
    ...
    if kind == AssetKind::Mcp {
        let Ok(mcp_path) = locate_mcp_json(default_root, project_root) else {
            return;
        };
        ...
        for plat in [
            ...
            if let Ok(adapter) = platform::for_id(plat) {
                let _ = mcp_json::remove_server_on_platform(&adapter.mcp_json_path(), name);
            }
        }
        return;
    }
    let Ok(src) = locate_source(default_root, project_root, kind, name) else {
        return;
    };
    for plat in [
        PlatformId::Cursor,
        ...
        let Some(dest) = compute_dest(plat, kind, name, &src) else {
            continue;
        };
...
/// 在 (project_root, default_root) 里定位一条资产的源路径(走 override 合并)。
fn locate_source(
    default_root: &Utf8Path,
    project_root: &Utf8Path,
    kind: AssetKind,
