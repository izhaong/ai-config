//! agents-manager 图形界面(Tauri 2 + React 19 前端)。
//!
//! Phase 3 W9:核心业务接入。
//!
...
use agents_manager_core::asset_scope;
use agents_manager_core::doctor;
use agents_manager_core::error::CoreError;
use agents_manager_core::git::{self, GitEnsureOutcome, GitRepoStatus, GitSyncConfig, GitSyncOutcome};
use agents_manager_core::hook;
use agents_manager_core::hook_adapter;
use agents_manager_core::materialize;
use agents_manager_core::mcp_json;
use agents_manager_core::model::{AssetKind, PlatformId, Project};
use agents_manager_core::paths;
use agents_manager_core::platform;
use agents_manager_core::platform_scan::{self, LinkState, PlatformAssetEntry, PlatformAssetList};
use agents_manager_core::source;
use agents_manager_core::sync::{agent_link_src, asset_dest_for_at_base};
use agents_manager_core::template::McpSyncState;
use agents_manager_store::Store;
use agents_manager_watcher::{dedupe_roots, start_debounced, WatchRoots, WatcherHandle};

use agents_manager_core::asset_ops::parse_skill_meta;
use agents_manager_core::asset_ops::AssetFileDetail;
...
impl AppState {
    fn new() -> Result<Self, String> {
        ...
        tracing::info!("agents-manager 资产根: {default_root}");
...
/// 健康检查（与 CLI `agents-manager doctor` 同源）。
#[tauri::command]
...
async fn cmd_import_to_source_plan(
    ...
    replace: Option<bool>,
) -> Result<agents_manager_core::projection::migration_action::ImportPlan, String> {
...
) -> Result<agents_manager_core::projection::migration_action::ImportApplyReport, String> {
    let kind = parse_kind(&kind)?;
    let source_platform = parse_deploy_plat(&from_platform)?;
    let (default_root, asset_root, deploy_base) = resolve_scope(&state, project.as_deref()).await?;
...
async fn cmd_platform_kind_paths(
    ...
    for plat in agents_manager_core::platform::ui_platform_ids() {
        let supported = if plat == PlatformId::AgentsManager {
            true
        } else {
            platform_supports(plat, k, &deploy_base)
        };
        let path = agents_manager_core::platform::kind_asset_path(plat, k, &deploy_base, &asset_root)
            .map(|p| p.to_string())
            .unwrap_or_default();
...
async fn cmd_skill_add_batch(
...
) -> Result<agents_manager_core::skills_add::SkillAddBatchOutcome, String> {
    if skills.is_empty() {
    ...
    let (default_root, asset_root, deploy_base) = resolve_scope(&state, project.as_deref()).await?;
    let items: Vec<agents_manager_core::skills_add::SkillAddBatchItem> = skills
        .into_iter()
        .map(|s| agents_manager_core::skills_add::SkillAddBatchItem {
            source: s.source,
...
/// 从平台已下发路径只读预览（无 agents-manager 源时供详情抽屉使用）。
#[tauri::command]
...
async fn cmd_projects_add(
    ...
    let project = tokio::task::spawn_blocking(move || -> Result<Project, String> {
        paths::ensure_asset_layout(&asset_for_layout)
            .map_err(|e| format!("初始化项目 .agents-manager 目录失败: {e}"))?;
        store
            .projects()
...
async fn cmd_git_sync(state: State<'_, AppState>) -> Result<GitSyncOutcome, String> {
    ...
    tokio::task::spawn_blocking(move || {
        let config = store.settings().git_config().map_err(|e| e.to_string())?;
        git::sync_repo(&root, &config, "chore: agents-manager 同步").map_err(|e| e.to_string())
    })
    .await
    .map_err(|e| format!("spawn_blocking join: {e}"))?
}
...
pub fn run() {
    tauri::Builder::default()
        ...
        .expect("error while running agents-manager GUI");
}
