...
describe("resolveEntryPlatformToggleAction", () => {
  it("已链接时收回", () => {
    expect(
      resolveEntryPlatformToggleAction(
        entry({ cursor: "linked", agentsmanager: "linked" }),
        "cursor",
        sourceView,
      ),
    ).toBe("retract");
  ...
  it("未链接时在源视图下发", () => {
    expect(
      resolveEntryPlatformToggleAction(
        entry({ cursor: "unlinked", agentsmanager: "linked" }),
        "cursor",
        sourceView,
      ),
    ).toBe("deploy");
  ...
  it("agentsmanager 已纳管时在其它平台视图收回 agents-manager 副本", () => {
    expect(
      resolveEntryPlatformToggleAction(
        entry({ agentsmanager: "linked" }),
        "agentsmanager",
        cursorView,
      ),
    ).toBe("retract");
  ...
  it("源视图浏览 agents-manager 时 mcp 点击 agents-manager icon 仅展示状态", () => {
    expect(
      resolveEntryPlatformToggleAction(
        { ...entry({ agentsmanager: "linked" }), kind: "mcp" },
        "agentsmanager",
        sourceView,
      ),
    ).toBe("skip");
  ...
  it("mcp 无源时从 Cursor 视图导入 agents-manager", () => {
    expect(
      resolveEntryPlatformToggleAction(
        {
          ...entry({ cursor: "synced", agentsmanager: "unlinked" }),
          kind: "mcp",
        },
        "agentsmanager",
        cursorView,
  ...
  it("平台视图不执行投影；只有源视图能打开 source-first 计划", () => {
    expect(
      resolveEntryPlatformToggleAction(
        entry({ agentsmanager: "linked", codex: "missing" }),
        "codex",
        cursorView,
      ),
    ).toBe("skip");
...
describe("resolveBatchPlatformToggleMode", () => {
  it("全部已激活 → retract_all", () => {
    ...
    expect(
      resolveBatchPlatformToggleMode(rows, "cursor", "agentsmanager", true),
    ).toBe("retract_all");
  });
  ...
  it("部分或全未激活 → activate_all", () => {
    ...
    const none = [
      entry({ cursor: "unlinked", agentsmanager: "linked" }),
      entry({ cursor: "missing", agentsmanager: "linked" }),
    ];
    expect(
      resolveBatchPlatformToggleMode(partial, "cursor", "agentsmanager", true),
    ).toBe("activate_all");
    expect(
      resolveBatchPlatformToggleMode(none, "cursor", "agentsmanager", true),
    ).toBe("activate_all");
  });
  ...
  it("源视图浏览 agents-manager 时批量不收回 agents-manager", () => {
    ...
    expect(
      resolveBatchPlatformToggleMode(rows, "agentsmanager", "agentsmanager", true),
    ).toBe("activate_all");
  });
...
describe("resolveBatchEntryPlatformAction", () => {
  it("activate_all：已激活项跳过，未激活项下发", () => {
    const linked = entry({ cursor: "linked", agentsmanager: "linked" });
    const unlinked = entry({ cursor: "unlinked", agentsmanager: "linked" });
    expect(
      resolveBatchEntryPlatformAction(
        linked,
        "cursor",
  ...
  it("retract_all：agents-manager 批量收回平台副本", () => {
    const linked = entry({ agentsmanager: "linked" });
    expect(
      resolveBatchEntryPlatformAction(
        linked,
        "agentsmanager",
