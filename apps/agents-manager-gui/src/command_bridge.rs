//! Tauri command 与 `agents-manager-core::asset_ops` 之间的薄桥。

use std::collections::{BTreeMap, BTreeSet};

use agents_manager_core::asset_ops::{self, AssetFileDetail, ScopeRoots};
use agents_manager_core::error::CoreError;
use agents_manager_core::model::{AssetKind, PlatformId};
use agents_manager_core::paths::SyncRoots;
...
use agents_manager_core::projection::lifecycle::build_projection_review;
...
use agents_manager_core::skills_add::{self, SkillAddBatchItem, SkillAddBatchOutcome, SkillsAddParams};
...
pub async fn import_to_source_apply(
    ...
    blocking(move || {
        ...
        apply_import_plan(
            ...
            &roots.deploy_base.join(".agents-manager-migrations"),
        )
    })
    .await
}
...
mod tests {
    ...
    async fn reviewed_projection_apply_requires_the_exact_plan_digest_and_creates_no_target_before_apply(
        ...
        let project_assets = root.join("repo/.agents-manager");
