 ...
 * Reads ~/.tauri/agents-manager.key unless TAURI_SIGNING_PRIVATE_KEY is already set.
 */
...
const defaultKeyPath = join(homedir(), ".tauri", "agents-manager.key");
...
function resolvePrivateKey() {
  ...
  if (!existsSync(keyPath)) {
    ...
    console.error(
      "[tauri:build] 生成命令: cd apps/agents-manager-gui && CI=true npm run tauri signer generate -- -w ~/.tauri/agents-manager.key -p \"\" -f",
    );
    console.error(
