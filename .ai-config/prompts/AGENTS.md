...
1. 本文件 `AGENTS.md`
2. `.ai-config/rules/00-ai-config-core.mdc`（核心工作流）
3. `.ai-config/rules/spec-kit-gate.mdc`（非平凡改动先 spec）
4. `.ai-config/rules/karpathy-guidelines.mdc`
5. 按任务读 `.ai-config/agents/*.md` 与 `.ai-config/skills/*/SKILL.md`
6. 编写 Hook 脚本 / `hooks.json` → `.ai-config/rules/hooks-asset-layout.mdc`
...
| `.ai-config/`             | 唯一项目 source：rules、agents、skills、commands、prompts、hooks |

## Agent 路由

| 任务                     | Agent                                  |
| ------------------------ | -------------------------------------- |
| Rust core / CLI / daemon | `.ai-config/agents/rust-ai-config-dev.md` |
| GUI React / Tauri 对接   | `.ai-config/agents/gui-ai-config-dev.md`  |
| 测试 / 验收 / 验证闭环   | `.ai-config/agents/qa-ai-config-dev.md`   |
...
Commands 索引：`.ai-config/commands/`（`/speckit.*`、`/ai-config-verify`、`/ai-config-sync`）
...
- 门禁与自动解析：`.ai-config/rules/spec-kit-gate.mdc`、`.ai-config/rules/gitflow-spec-kit.mdc`
