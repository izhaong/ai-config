...
多 IDE 共享的 **配置管理工具**仓库。用户 skills/rules/commands/MCP 在 **`~/.agents/`**，不在本仓。

## 加载顺序

1. 本文件 `AGENTS.md`
2. `.agents/rules/00-agents-manager-core.mdc`（核心工作流）
3. `.agents/rules/spec-kit-gate.mdc`（非平凡改动先 spec）
4. `.agents/rules/karpathy-guidelines.mdc`
5. 按任务读 `.agents/agents/*.md` 与 `.agents/skills/*/SKILL.md`
6. 编写 Hook 脚本 / `hooks.json` → `.agents/rules/hooks-asset-layout.mdc`
...
| `.agents/`             | 唯一项目 source：rules、agents、skills、commands、prompts、hooks |

## Agent 路由

| 任务                     | Agent                                  |
| ------------------------ | -------------------------------------- |
| Rust core / CLI / daemon | `.agents/agents/rust-agents-manager-dev.md` |
| GUI React / Tauri 对接   | `.agents/agents/gui-agents-manager-dev.md`  |
| 测试 / 验收 / 验证闭环   | `.agents/agents/qa-agents-manager-dev.md`   |
...
Commands 索引：`.agents/commands/`（`/speckit.*`、`/agents-manager-verify`、`/agents-manager-sync`）
...
- 门禁与自动解析：`.agents/rules/spec-kit-gate.mdc`、`.agents/rules/gitflow-spec-kit.mdc`
