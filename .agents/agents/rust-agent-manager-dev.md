...
- 在 **`agents-manager-core`** 实现/修改同步、平台适配、链接、模板、资产模型
- 复用 workspace / 成熟 crate（itertools、regex 等），勿重复实现 — 见 `rust-agents-manager.mdc`
...
- GUI 需新 Tauri 命令时，与 **gui-agents-manager-dev** 对齐 command 名与 DTO
- 交付流程见 skill：`agents-manager-delivery`
