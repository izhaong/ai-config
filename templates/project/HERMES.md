...
| 层级 | 路径 |
| --- | --- |
| 全局通用 | `~/.agent-manager/rules/*.mdc` |
| 本项目 | `.cursor/rules/*.mdc`（`.hermes` 安装的全局 rules 与 agent-manager 同源） |
...
- 全局：`~/.agent-manager/skills/`（`agent-manager install` / `agent-manager sync` 下发到各 IDE）
- 项目：`.cursor/skills/`
